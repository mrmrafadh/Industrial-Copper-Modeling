...
class options:
    ...
    status_dict = {'Lost':1, 'Won':7, 'Draft':0, 'To be approved':6, 'Not lost for AM':2,
...
class prediction:
    ...
    def regression():
        ...
        if button:
            ...
            with open(r'model_reggression.pkl', 'rb') as f:
                model = pickle.load(f)
            # make array for all user input values in required order for model prediction
            user_data = np.array([[np.log(float(quantity_log)),
    ...
    def classification():
        ...
        if button:
            ...
            with open(r'model_classifier.pkl', 'rb') as f:
                model = pickle.load(f)
